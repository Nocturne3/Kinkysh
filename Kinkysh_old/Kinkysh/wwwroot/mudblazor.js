...
window.mudInputAutoGrow = {
    initAutoGrow: (elem,maxLines)=>{
        const compStyles = getComputedStyle(elem)
            , lineHeight = parseFloat(compStyles.getPropertyValue("line-height"));
        let minHeight = lineHeight * elem.rows
            , maxHeight = 0;
        maxLines > 0 && (maxHeight = lineHeight * maxLines);
        elem.adjustAutoGrowHeight = function() {
            elem.style.height = 0;
            let newHeight = Math.max(minHeight, elem.scrollHeight);
            maxHeight > 0 && (newHeight = Math.min(newHeight, maxHeight));
    ...
    ,
    adjustHeight: elem=>{
        typeof elem.adjustAutoGrowHeight == "function" && elem.adjustAutoGrowHeight()
    }
};
...
document.addEventListener("click", function(event) {
    ...
    target && setRippleOffset(event, target)
});
...
class MudWindow {
    ...
    updateStyleProperty(elementId, propertyName, value) {
        ...
        element && element.style.setProperty(propertyName, value)
    }
